# Ultrasonic Radar with servo motor interrupts version (HC-SR04)

This project demonstrates how to use **Zephyr** on an **ESP32 DevKitC WROOM** to:

* Sweep a **servo motor** using PWM
* Measure distance using an **HC-SR04 ultrasonic sensor** with **interrupts**
...
* **Subsystems**:
  * PWM (Servo control)
  * GPIO (Trigger/Echo)
  * Interrupt handling for HC-SR04
  * k_work queue for processing measurements
  * printk logging
...
* At each angle:
  ...
  * Uses interrupts on the Echo pin to record pulse start and stop times

  * Calculates distance in cm in a k_work handler

  * Prints `Angle` and `Distance` to serial
### Interrupt Flow:
* `echo_isr()` is triggered on both rising and falling edges

  * Rising edge → records `start_time`

  * Falling edge → records `stop_time`

* `k_work` processes the measurement and prints results
...
* Ensure ISR is configured correctly for both edges
